...
from meteor.plugins.bases import OncallPlugin
...
def get_active_instance(*, db_session, plugin_type: str, project_id=None) -> Optional[PluginInstance]:
...
def get_active_instances(*, db_session, plugin_type: str, project_id=None) -> Optional[PluginInstance]:
    """Fetches the current active plugin for the given type."""
    return (
        db_session.query(PluginInstance)
        .join(Plugin)
...
def get_active_instance_by_slug(*, db_session, slug: str, project_id=None) -> Optional[PluginInstance]:
    """Fetches the current active plugin for the given type."""
    return (
        db_session.query(PluginInstance)
        .join(Plugin)
...
def get_enabled_instances_by_type(*, db_session, project_id: int, plugin_type: str) -> List[Optional[PluginInstance]]:
