...
from meteor.auth.models import MeteorUser, MeteorUserOrganization
from meteor.database.core import engine
...
def add_user(
    *,
    ...
    db_session.add(MeteorUserOrganization(meteor_user_id=user.id, organization_id=organization.id, role=role))
    db_session.commit()
